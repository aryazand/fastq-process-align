...
rule samtools_filter:
    ...
    shell:
        """
        samtools view -bh {params.extra} -T {input.fa} -@ {threads} -o {output.bam} {input.cram} {params.region} 2> {log.view}
        """
...
rule sort_umitools_input:
    ...
    output:
        temp("results/processed_alignment/prep_for_dedup/{sample}_sorted.bam"),
    log:
...
rule index_umitools_input:
    ...
    output:
        temp("results/processed_alignment/prep_for_dedup/{sample}_sorted.bam.bai"),
    log:
